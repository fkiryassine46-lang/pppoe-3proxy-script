...
# Ligne vide avant l'affichage de la barre "Checking licence"
printf "\n"

# Barre de progression ORANGE
printf "Checking licence... ${ORANGE}[%*s]${RESET}" "$BAR_WIDTH" ""
...
while kill -0 "$LIC_PID" 2>/dev/null; do
    ...
    printf "\rChecking licence... ${ORANGE}[%s%s]${RESET}" "$filled" "$empty"
    sleep 0.1
done
...
printf "\rChecking licence... ${ORANGE}[%s]${RESET}\n" "$filled"
...
progress_bar() {
    ...
    # Barre + pourcentage dans la même couleur
    printf "\r%s %b[%-*s] %3d%%%b" \
        "$label" "$color" "$width" "${bar_filled}${bar_empty}" "$percent" "$RESET"
}
...
EOF
    ...
    progress_bar "$PH1_DONE" "$COUNT" "Phase 1" "$GREEN"
done
...
for i in $(seq 0 $((COUNT - 1))); do
    ...
    PH2_DONE=$((PH2_DONE + 1))
    # Barre rose pour la phase 2
    progress_bar "$PH2_DONE" "$COUNT" "Phase 2" "$PINK"
done
