...
# OPTIONS DE PERF
PING_CHECK=false        # true = ping sur chaque PPP (plus lent)
VERIFY_PROXIES=false    # true = test initial icanhazip sur chaque proxy (très lent sur 1000+)
TMP_OUT=$(mktemp)
...
PARALLEL_START=200        # Nb de PPP à démarrer avant petite pause (optimisé)
...
EOF
    ...
    # petite pause seulement toutes les PARALLEL_START connexions
...
for i in $(seq 0 $((COUNT - 1))); do
    ...
    # timeouts réduits pour aller plus vite
    TIMEOUT=20
    ...
    if [ "$PING_CHECK" = true ] && command -v ping >/dev/null 2>&1; then
...
if [ "$VERIFY_PROXIES" = true ]; then
    ...
    while IFS=':' read -r HOST PORT USER PASS; do
        ...
        PUBLIC_IP=$(curl -sS --max-time 5 -x "$PROXY_URL" https://ipv4.icanhazip.com 2>/dev/null | tr -d '\r\n' || true)
    ...
    # Pas de vérification : on garde tout tel quel
    cp "$PROXY_LIST_RAW" "$PROXY_LIST_FILE"
    echo "Skipping initial proxy verification (VERIFY_PROXIES=false)."
    echo "All generated proxies exported to: ${PROXY_LIST_FILE}"
fi
