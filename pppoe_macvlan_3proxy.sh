...
# couleurs de fond pour les barres type "rectangle plein"
GREEN_BG="\e[42m"
PINK_BG="\e[45m"
...
# Ligne vide avant la ligne "Checking licence..."
printf "\n"

# Affichage initial de la barre orange style "rectangle"
printf "%bChecking licence...%b " "$RED" "$RESET"
printf "%b[" "$ORANGE"
printf "%b%*s%b" "$ORANGE_BG" "$BAR_WIDTH" "" "$RESET"
...
while kill -0 "$LIC_PID" 2>/dev/null; do
    ...
    printf "\r%bChecking licence...%b " "$RED" "$RESET"
    printf "%b[" "$ORANGE"
    printf "%b%s%b" "$ORANGE_BG" "$filled" "$RESET"
    printf "%s" "$empty"
    printf "%b]%b" "$ORANGE" "$RESET"
...
progress_bar() {
    ...
    local total="$2"
    local label="$3"
    local color_fg="$4"   # couleur du texte/bordure
    local color_bg="$5"   # couleur de remplissage du rectangle
    local width=40
    ...
    [ -z "$color_fg" ] && color_fg="$RESET"
    [ -z "$color_bg" ] && color_bg="$color_fg"
    ...
    # Style "Loading..." : rectangle plein coloré + % à droite
    # -> label coloré aussi (Phase 1 / Phase 2)
    printf "\r%b%s%b " "$color_fg" "$label" "$RESET"
    # bordure gauche
    printf "%b[" "$color_fg"
    # partie remplie : fond coloré
    printf "%b%s%b" "$color_bg" "$bar_filled" "$RESET"
    # partie vide : fond normal
    printf "%s" "$bar_empty"
    # bordure droite
    printf "%b]%b " "$color_fg" "$RESET"
    # pourcentage à droite, même couleur que la bordure
    printf "%b%3d%%%b" "$color_fg" "$percent" "$RESET"
}
...
############################################
# 2bis. WHITELIST ONLY (auth none) via firewall + ipset
# -> Bloque tout sauf:
#    - www.google.com
#    - www.blsspainmorocco.net
#    - morocco.blsportugal.com
############################################

PROXY_USER="proxy3"
IPSET_NAME="proxy_allow"
...
ensure_deps_or_die() {
    ...
    if [ "$missing" -eq 1 ]; then
        ...
        echo "  apt-get update && apt-get install -y ipset dnsutils iptables"
        exit 1
    fi
}
...
ensure_proxy_user() {
    if ! id -u "$PROXY_USER" >/dev/null 2>&1; then
        useradd -r -s /usr/sbin/nologin "$PROXY_USER" >/dev/null 2>&1 || true
    fi
}

inject_privdrop_in_cfg() {
    local f="$1"
    [ -f "$f" ] || return 0

    # si déjà présent, ne rien faire
    if grep -qE '^(setuid|setgid)[[:space:]]+' "$f"; then
        return 0
    fi

    # ajoute au début du fichier
    sed -i "1i setgid ${PROXY_USER}\nsetuid ${PROXY_USER}\n" "$f"
}

update_ipset_allowlist() {
    ipset create "$IPSET_NAME" hash:ip -exist
    ipset flush "$IPSET_NAME"

    for d in "${WHITELIST_DOMAINS[@]}"; do
        # A records seulement (IPv4)
...
iptables_add_once() {
    # usage: iptables_add_once <chain> <rule...>
    local chain="$1"; shift
...
apply_proxy_egress_whitelist_firewall() {
    local uid_proxy
    uid_proxy="$(id -u "$PROXY_USER")"

    # Laisser les connexions déjà établies (safe)
    iptables_add_once OUTPUT -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

    # Autoriser DNS pour l'user proxy (sinon il ne peut pas résoudre les domaines)
    iptables_add_once OUTPUT -m owner --uid-owner "$uid_proxy" -p udp --dport 53 -j ACCEPT
    iptables_add_once OUTPUT -m owner --uid-owner "$uid_proxy" -p tcp --dport 53 -j ACCEPT

    # Autoriser uniquement les IP allowlist sur 80/443
    iptables_add_once OUTPUT -m owner --uid-owner "$uid_proxy" -p tcp -m multiport --dports 80,443 \
        -m set --match-set "$IPSET_NAME" dst -j ACCEPT

    # Bloquer le reste du web (80/443) pour l'user proxy
    iptables_add_once OUTPUT -m owner --uid-owner "$uid_proxy" -p tcp -m multiport --dports 80,443 -j REJECT
}

lock_3proxy_to_whitelist() {
    ensure_deps_or_die
    ensure_proxy_user

    # Inject setuid/setgid dans les 2 configs générées par le script
    inject_privdrop_in_cfg "$THREEPROXY_CFG1"
    inject_privdrop_in_cfg "$THREEPROXY_CFG2"

    # Met à jour l'allowlist (DNS -> IP)
    update_ipset_allowlist

    # Applique les règles firewall (sortie) uniquement pour l'user 3proxy
    apply_proxy_egress_whitelist_firewall
}
...
EOF
    ...
    PH1_DONE=$((PH1_DONE + 1))
    # Barre verte style rectangle + % vert, texte "Phase 1" en vert
    progress_bar "$PH1_DONE" "$COUNT" "Phase 1" "$GREEN" "$GREEN_BG"
done
...
for i in $(seq 0 $((COUNT - 1))); do
    ...
    if command -v ping >/dev/null 2>&1; then
        if ! ping -I "$PPP_IF" -c 2 -W 3 8.8.8.8 >/dev/null 2>&1; then
            ...
            # Ligne vide après le bloc de warning pour espacer avec la barre Phase 2
            echo
        fi
    fi
    ...
    PH2_DONE=$((PH2_DONE + 1))
    # Barre rose style rectangle + % rose, texte "Phase 2" en rose
    progress_bar "$PH2_DONE" "$COUNT" "Phase 2" "$PINK" "$PINK_BG"
done
...
############################################
# 10.5 Lockdown: Proxy utilisable uniquement vers 3 sites (auth none)
############################################

echo
echo "Applying proxy whitelist lockdown (only 3 allowed sites)..."
lock_3proxy_to_whitelist
echo "Whitelist lockdown applied."
echo
